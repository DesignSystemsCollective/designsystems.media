...
import { allPostsFilteredAndSorted } from "../src/utils/mediaCollection";
...
async function createOffsetMosaic(
    ...
    if (imagePaths.length === 0) {
        console.warn(`[Mosaic Creation] No images provided for ${path.basename(outputPath)}. Skipping.`);
        return;
    }
    console.log(`[Mosaic Creation] Starting offset mosaic for ${path.basename(outputPath)} (${finalWidth}x${finalHeight}) using ${imagePaths.length} images...`);
        ...
        for (let col = 0; col < colsToLayOut + 2; col++) {
            ...
            try {
                const resizedImageBuffer = await sharp(currentImagePath)
                    .resize(actualBaseTileWidth, actualBaseTileHeight, { fit: 'cover', position: 'center' })
                    .toBuffer();
                ...
                console.error(`[Mosaic Creation] Error processing individual image "${currentImagePath}":`, imageProcessingError.message);
            }
            imageIndex++;
        }
    }
    ...
    if (compositeOperations.length === 0) {
        console.error(`[Mosaic Creation] No valid images were successfully processed for compositing for ${path.basename(outputPath)}. Cannot create mosaic.`);
        return;
    }
    ...
    const intermediateCanvasWidth = Math.max(calculatedIntermediateWidth, requiredExtractWidth);
    const intermediateCanvasHeight = Math.max(calculatedIntermediateHeight, requiredExtractHeight);

    if (intermediateCanvasWidth <= 0 || intermediateCanvasHeight <= 0) {
        console.error(`[Mosaic Creation] Calculated intermediate canvas dimensions are invalid for ${path.basename(outputPath)}`);
        return;
    }
    ...
    try {
        ...
        if (overlayOptions && overlayOptions.imagePath) {
            try {
                ...
                const overlayTop = Math.round((finalHeight - overlayOptions.height) / 2);
                ...
                console.log(`[Mosaic Creation] Added overlay image: ${path.basename(overlayOptions.imagePath)}`);
            } catch (error) {
                console.warn(`[Mosaic Creation] Warning: Could not load overlay image at ${overlayOptions.imagePath}:`, error.message);
            }
        }
        ...
        console.error(`[Mosaic Creation] Error during mosaic creation for ${path.basename(outputPath)}:`, error);
        throw error;
    }
}
...
export async function runAllMosaics() {
    ...
    const sortedPosts = allPosts.sort((a, b) => b.data.publishedAt.getTime() - a.data.publishedAt.getTime());
    ...
    for (const post of sortedPosts.slice(0, maxImagesToPull)) {
        ...
        const absolutePostDir = path.dirname(path.join(postContentBasePath, post.id));
        ...
        try {
            ...
            console.warn(`[Astro Mosaics] Warning: Image file not found at path: ${absoluteImagePathForSharp} for post "${post.data.title}". Skipping.`);
        }
    }

    if (recentPostImagePaths.length < 15) {
        console.error(`[Astro Mosaics] ERROR: Only found ${recentPostImagePaths.length} recent post images suitable for mosaics.`);
        throw new Error('Not enough source images to generate mosaics. Build failed.');
    }

    console.log(`[Astro Mosaics] Found ${recentPostImagePaths.length} valid recent post images.`);
    ...
    if (!overlayPath) {
        console.warn(`[Astro Mosaics] Warning: Could not find DSMoverlay.png in any of these locations:`);
